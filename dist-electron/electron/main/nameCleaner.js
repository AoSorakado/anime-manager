...
function preserveTitleBrackets(value) {
    return value.replace(/\[([^\]]+)\]/g, (token, content) => {
        if (isNoiseToken(content))
            return " ";
        return ` ${content} `;
    });
}
function bracketContent(token) {
    const content = token.slice(1, -1).trim();
    return isNoiseToken(content) ? " " : ` ${content} `;
}
function isNoiseToken(value) {
    const lower = value.toLowerCase();
    if (/^\d{1,3}\s*-\s*\d{1,3}$/.test(lower))
        return true;
    if (/\d{1,3}\s*-\s*\d{1,3}/.test(lower))
        return true;
    if (/^\d{1,3}$/.test(lower))
        return true;
    if (lower.includes("全集") || lower.includes("合集"))
        return true;
    if (groupWords.some((word) => lower.includes(word)))
        return true;
...
export function simpleMatchScore(query, title) {
