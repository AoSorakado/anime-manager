...
export async function scanLocalSource(sourceId, videoExtensions = defaultVideoExtensions) {
    const source = getSource(sourceId);
    if (!source)
        throw new Error(`Source ${sourceId} not found`);
    if (source.type !== "local")
        throw new Error("MVP only supports local sources");
  ...
  `);
    ...
    if (cleanupAllowed) {
        ...
        log("warning", "scanner", `跳过旧条目清理：本次扫描结果异常偏少`, `扫描前 ${beforeItemCount} 个条目，本次发现 ${foundMediaItems.length} 个。可能是挂载盘、CloudDrive 或网络盘尚未完全加载。旧条目已保留。`);
    }
    ...
    async function collectCollectionChildren(collectionPath) {
...
export function isAuxiliaryFolder(folderName) {
    ...
    if (tokens.some((token) => specialTokens.has(token)))
        return true;
    if (/(^|[^a-z])(ncop|nced|op|ed|sp|sps)([^a-z]|$)/i.test(normalized))
        return true;
    if (/(oped|opand-ed|opended|ncoped)/i.test(compact))
        return true;
    return /(特典|映像特典|菜单|字体|扫图|扫描|字幕|无字幕|特报|预告|预告片|片头|片尾)/.test(compact);
}
export function isAuxiliaryVideoFile(fileName) {
    const baseName = path.basename(fileName, path.extname(fileName))
        .toLowerCase()
        .replace(/[Ａ-Ｚａ-ｚ０-９]/g, (char) => String.fromCharCode(char.charCodeAt(0) - 0xfee0));
    const compact = baseName.replace(/[^a-z0-9\u4e00-\u9fff]+/g, "");
    if (/(menu|menus|ncop|nced|oped|opand?ed|pv|cm|preview|trailer)/i.test(compact))
        return true;
    if (/(菜单|特典|映像特典|片头|片尾|预告|预告片|特报|无字幕)/.test(compact))
        return true;
...
export function isCollectionFolder(folderName) {
    const normalized = folderName
        .toLowerCase()
        .replace(/[Ａ-Ｚａ-ｚ０-９]/g, (char) => String.fromCharCode(char.charCodeAt(0) - 0xfee0));
    const compact = normalized.replace(/[^a-z0-9\u4e00-\u9fff]+/g, "");
