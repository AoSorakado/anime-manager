...
export function listPikpakOfflineTasks() {
    return listCloudOfflineTasks(100).filter((task) => task.provider === "pikpak");
}
export async function submitPikpakOfflineDownload(input) {
    const url = input.url.trim();
    if (!url)
        throw new Error("没有可提交给 PikPak 的链接");
...
export function refreshPikpakOfflineTasks() {
    return listPikpakOfflineTasks();
}
function joinRcloneRemote(remoteInput, folderInput) {
...
function runRclone(command, args) {
