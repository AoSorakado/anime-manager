...
export async function addWebDavSourceAndTest(input) {
    const source = addWebDavSource(input);
    await listWebDavDirectory(source, source.root_path);
    return source;
}
export async function scanWebDavSource(sourceId, videoExtensions = defaultVideoExtensions) {
    const source = getSource(sourceId);
    if (!source)
        throw new Error(`Source ${sourceId} not found`);
    if (source.type !== "webdav")
        throw new Error("不是 WebDAV 媒体库");
    ...
    async function collectVideos(currentPath, depth, files) {
        if (depth > maxDepth)
            return;
    ...
    for (const entry of rootEntries) {
        if (!entry.isDirectory || entry.name.startsWith(".") || isAuxiliaryFolder(entry.name))
            continue;
        if (isCollectionFolder(entry.name)) {
            const children = await safeListWebDavDirectory(sourceRecord, entry.path);
            for (const child of children) {
                if (!child.isDirectory || child.name.startsWith(".") || isAuxiliaryFolder(child.name))
                    continue;
  ...
  `);
    ...
    db.transaction(() => {
        for (const itemData of foundMediaItems) {
            const timestamp = now();
            const item = upsertItem.get(sourceId, itemData.folderPath, itemData.folderName, cleanFolderName(itemData.folderName), timestamp, timestamp, timestamp);
            ...
            const existingFiles = listExistingFiles.all(item.id);
    ...
    if (cleanupAllowed) {
        ...
        log("warning", "webdav", "跳过 WebDAV 旧条目清理：本次扫描结果异常偏少", `扫描前 ${beforeItemCount} 个条目，本次发现 ${foundMediaItems.length} 个。可能是 WebDAV 服务端目录列表未完整返回，旧条目已保留。`);
    }
    ...
    async function addMediaFolder(folderPath, folderName) {
        const files = [];
...
async function safeListWebDavDirectory(source, directoryPath) {
    try {
        const entries = await listWebDavDirectory(source, directoryPath);
        if (entries.length === 0)
            log("warning", "webdav", `WebDAV 目录为空或未返回子项：${directoryPath}`);
        return entries;
    }
    catch (error) {
...
export async function uploadWebDavSyncState() {
...
export async function downloadWebDavSyncState() {
    const settings = getSettings();
    const syncTarget = syncSettingsToSource(settings);
...
async function listWebDavDirectory(source, directoryPath) {
...
async function putWebDavFile(source, targetPath, body) {
...
async function getWebDavFile(source, targetPath) {
    const response = await fetch(buildRequestUrl(source, targetPath), {
        method: "GET",
        headers: { Authorization: authHeader(source) }
    });
    if (!response.ok)
        throw new Error(`WebDAV 拉取同步文件失败：${response.status} ${response.statusText}`);
    return response.text();
}
async function ensureWebDavDirectory(source, directoryPath) {
...
function parseMultiStatus(xml, basePath, requestedPath) {
...
function normalizeResponsePath(decodedPath, normalizedBase, normalizedRequested) {
    ...
    if (normalizedRequested !== "/" && !isPathWithin(normalizedPath, normalizedRequested)) {
        ...
        if (index >= 0)
            normalizedPath = normalizeDavPath(normalizedPath.slice(index));
    }
...
function decodeDavHrefPath(href) {
...
function buildPlaybackUrl(source, filePath) {
    const url = new URL(buildRequestUrl(source, filePath));
    if (source.username)
        url.username = source.username;
    if (source.encrypted_password)
        url.password = source.encrypted_password;
    return url.toString();
}
function buildRequestUrl(source, davPath) {
...
function syncSettingsToSource(settings) {
    if (!settings.syncWebdavUrl?.trim())
        throw new Error("先填写同步 WebDAV 地址");
    return {
        id: 0,
        name: "同步 WebDAV",
        type: "webdav",
...
function countSourceMediaItems(sourceId) {
    const row = getDb().prepare("SELECT COUNT(*) AS count FROM media_items WHERE source_id = ?").get(sourceId);
    return Number(row.count || 0);
}
function shouldCleanupStaleItems(beforeItemCount, foundItemCount) {
    if (foundItemCount === 0)
        return beforeItemCount === 0;
    if (beforeItemCount < 20)
        return true;
    return foundItemCount >= Math.max(10, Math.floor(beforeItemCount * 0.65));
}
function createSourceSnapshot(sourceId) {
    const rows = getDb()
        .prepare(`
...
function deleteStaleMediaItems(sourceId, currentFolderPaths) {
    const db = getDb();
    ...
    const staleRows = db.prepare(`SELECT id FROM media_items WHERE ${where}`).all(...params);
