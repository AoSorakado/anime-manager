...
import { addRssSubscription, deleteRssSubscription, getRssItem, getRssSubscription, getSettings, listRssItems, listRssSubscriptions, log, markRssItemDownloadResult, upsertRssItems } from "./db.js";
export function subscriptionsList() {
    return listRssSubscriptions();
}
export function subscriptionItems(subscriptionId) {
    return listRssItems(subscriptionId);
}
export function subscriptionAdd(input) {
...
export async function searchMikan(keyword) {
...
export async function getMikanBangumi(bangumiId) {
    ...
    if (!response.ok)
        throw new Error(`蜜柑详情页获取失败：${response.status}`);
...
export async function sendUrlToQbittorrent(url, savePath, title, seriesTitle) {
    const target = url.trim();
    if (!target)
        throw new Error("没有可提交给 qBittorrent 的链接");
    await qbAddUrl(target, savePath, title, seriesTitle);
    log("info", "subscription", `已提交临时任务到 qBittorrent：${title || target}`, target);
    return { sent: true };
}
export async function sendPendingItemsToQbittorrent(subscriptionId) {
...
export async function testQbittorrent() {
    ...
    if (!response.ok)
        throw new Error(`qBittorrent 连接失败：${response.status} ${response.statusText}`);
...
async function fetchFeed(feedUrl) {
    ...
    if (!response.ok)
        throw new Error(`RSS 获取失败：${response.status} ${response.statusText}`);
...
async function fetchMikanSearchPage(keyword) {
    ...
    if (!response.ok)
        throw new Error(`蜜柑搜索页获取失败：${response.status} ${response.statusText}`);
    return parseMikanSearchHtml(await response.text());
}
function parseMikanBangumiHtml(html) {
    const items = [];
    ...
    while ((subgroupMatch = subgroupRegex.exec(html)) !== null) {
        ...
        for (const row of rows) {
            if (!row.includes("magnet-link-wrap"))
                continue;
...
function parseMikanSearchHtml(html) {
    const rows = html.match(/<tr\b[\s\S]*?<\/tr>/gi) || [];
    const items = [];
    for (const row of rows) {
        const cells = row.match(/<td\b[\s\S]*?<\/td>/gi) || [];
        if (cells.length < 3 || !/Download|download|torrent|磁链|glyphicon-download|fa-download/i.test(row))
            continue;
        ...
        if (!title || /番组名|大小|更新时间/.test(title))
            continue;
...
function mergeSearchItems(primary, fallback) {
    ...
    for (const item of primary)
        result.set(normalizeTitleKey(item.title), item);
...
function parseSubtitleGroup(title) {
    const match = title.match(/^[\[\【]([^\]\】]{1,40})[\]\】]/);
    return match?.[1]?.trim() || null;
}
function parseSizeText(title) {
...
function buildMikanSearchPage(keyword) {
    const base = (getSettings().mikanBaseUrl || "https://mikanani.me").replace(/\/+$/, "");
    return `${base}/Home/Search?searchstr=${encodeURIComponent(keyword)}`;
}
function absolutizeMikanUrl(url) {
...
async function qbAddUrl(url, savePath, title, seriesTitle) {
    await ensureQbittorrentAvailable();
    const cookie = await qbLogin();
    ...
    if (targetPath.trim())
        params.set("savepath", targetPath.trim());
    ...
    if (!response.ok)
        throw new Error(`qBittorrent 添加任务失败：${response.status} ${response.statusText}`);
    const text = await response.text();
    if (/fails?|error/i.test(text))
        throw new Error(`qBittorrent 添加任务失败：${text}`);
}
function buildGroupedSavePath(basePath, title) {
    ...
    if (!folderName)
        return base;
    if (/^[a-z]+:\/\//i.test(base))
        return `${base.replace(/[\\/]+$/, "")}/${encodeURIComponent(folderName)}`;
    if (/^[a-zA-Z]:[\\/]/.test(base) || base.includes("\\"))
        return path.win32.join(base, folderName);
    return `${base.replace(/\/+$/, "")}/${folderName}`;
}
function guessSeriesFolderName(title) {
    let value = title
        .replace(/^[\[\【][^\]\】]{1,60}[\]\】]\s*/g, "")
        .replace(/\[\s*复制磁链\s*\]|\(\s*复制磁链\s*\)|【\s*复制磁链\s*】|复制磁链/gi, "")
...
async function ensureQbittorrentAvailable() {
    if (await isQbittorrentReachable(1200))
        return;
    const settings = getSettings();
...
async function waitForQbittorrent(timeoutMs) {
...
async function isQbittorrentReachable(timeoutMs) {
