...
// ──── Public API ────
export async function getSeasonAnime(year, season, options) {
...
// ──── Fetch from Bangumi ────
export async function fetchSeasonFromBangumi(year, season) {
...
export function normalizeBangumiSubject(raw) {
    const images = raw.images || {};
    const rating = raw.rating || {};
    const airDate = raw.date || null;
    ...
    return {
        ...
        tags: Array.isArray(raw.tags) ? raw.tags.map((t) => ({ name: t.name || "", count: t.count || 0 })) : [],
        raw,
    };
}
function delay(ms) {
