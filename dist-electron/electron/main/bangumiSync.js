        ...
        while (true) {
            ...
            for (const raw of page.data) {
                ...
                allEntries.push({
                    ...
                    subject_eps: subj?.eps != null ? subj.eps : raw.subject?.eps_total,
...
export async function fetchBangumiServiceStatus() {
    ...
    try {
        ...
        if (response.ok) {
            ...
            return { ...parseStatusFeed(xml), source: "feed" };
        }
...
async function probeBangumiEndpoints() {
    ...
    const probes = [
        ...
        },
        {
            ...
            validate: (resp, text) => resp.ok && (() => { try {
    ...
    const components = [];
    const results = await Promise.allSettled(probes.map(async (ep) => {
    ...
    const overall = outageCount > 0 ? "outage" : degradedCount > 0 ? "degraded" : "operational";
