...
export async function searchBangumi(mediaItemId: number, keyword?: string, enrichDetails = true) {
  ...
  if (!item) throw new Error(`Media item ${mediaItemId} not found`);
  ...
  const detailedSubjects: Record<string, Record<string, unknown>> = {};
  for (const subject of rawSubjects.slice(0, detailLimit)) {
    const id = String(subject.id);
    const detail = await fetchBangumiSubjectDetail(id);
    if (detail) detailedSubjects[id] = detail;
    await delay(180);
  }
  const candidates: BangumiCandidate[] = rawSubjects.map((subject) => normalizeCandidate(detailedSubjects[String(subject.id)] || subject, query));
...
export async function batchSearchBangumi(options?: { unmatchedOnly?: boolean; autoApplyThreshold?: number; delayMs?: number }): Promise<BatchScrapeResult> {
  ...
  const result: BatchScrapeResult = { total: items.length, searched: 0, autoApplied: 0, needConfirm: 0, failed: 0 };
...
export async function batchRefreshBangumiById(options?: { delayMs?: number }): Promise<BatchIdRefreshResult> {
  const delayMs = options?.delayMs ?? 900;
  const items = listMediaItems("", "created_at", "all").filter((item) => item.provider === "bangumi" && item.external_id);
  const result: BatchIdRefreshResult = { total: items.length, refreshed: 0, failed: 0 };
...
function bestCandidate(candidates: BangumiCandidate[], item: MediaItem) {
...
function normalizeCandidate(subject: Record<string, unknown>, query: string): BangumiCandidate {
  const images = subject.images as Record<string, string> | undefined;
  const rating = subject.rating as { score?: number; rank?: number; total?: number } | undefined;
...
async function fetchBangumiSubjectDetail(subjectId: string) {
...
function extractAliases(infobox: unknown) {
  if (!Array.isArray(infobox)) return [];
  return (infobox as Array<{ key?: string; value?: unknown }>)
    .filter((entry) => String(entry.key || "").includes("别名"))
    .flatMap((entry) => Array.isArray(entry.value) ? entry.value : [entry.value])
    .map((value) => typeof value === "object" && value !== null && "v" in value ? String((value as { v: unknown }).v) : String(value || ""))
    .filter(Boolean);
}
...
export async function applyBangumiCandidate(mediaItemId: number, externalId: string) {
  const candidate = getDb()
    .prepare("SELECT * FROM metadata_candidates WHERE media_item_id = ? AND provider = 'bangumi' AND external_id = ?")
    .get(mediaItemId, externalId) as { raw_json: string; title: string; original_title?: string; year?: number; score?: number; cover_url?: string; external_id: string } | undefined;
  if (!candidate) {
    return refreshBangumiById(mediaItemId, externalId);
  }

  const rawCandidate = JSON.parse(candidate.raw_json || "{}") as Record<string, unknown>;
  ...
  const rating = raw.rating as { score?: number; rank?: number } | undefined;
  ...
  getDb()
    ...
    `)
    .run(
      String(raw.name_cn || candidate.title),
      String(raw.name || candidate.original_title || ""),
      preferChineseSummary(String(raw.summary || "")),
      coverPath,
      candidate.year ?? null,
      String(raw.date || ""),
      rating?.score ?? candidate.score ?? null,
      rating?.rank ?? null,
      JSON.stringify(raw),
      JSON.stringify(extractTags(raw)),
...
export async function refreshBangumiById(mediaItemId: number, externalId?: string) {
  const item = getMediaItem(mediaItemId);
  const subjectId = String(externalId || item?.external_id || "").trim();
  if (!item || !subjectId) throw new Error("Bangumi ID not found");
  const raw = await fetchBangumiSubjectDetail(subjectId);
  if (!raw) throw new Error(`Bangumi detail not found: ${subjectId}`);
  ...
  getDb()
    ...
    .run(
      ...
      preferChineseSummary(String(raw.summary || "")),
      coverPath,
      String(raw.date || "").slice(0, 4) ? Number(String(raw.date || "").slice(0, 4)) : null,
      String(raw.date || ""),
      rating?.score ?? null,
      rating?.rank ?? null,
      JSON.stringify(raw),
...
export async function getBangumiSubjectDetail(subjectId: string): Promise<BangumiSubjectDetail | null> {
  ...
  if (!raw) return null;
  const images = raw.images as Record<string, string> | undefined;
  const rating = raw.rating as { total?: number; score?: number; rank?: number; count?: Record<string, number> } | undefined;
  ...
  return {
    ...
    infobox: Array.isArray(raw.infobox)
      ? (raw.infobox as Array<{ key?: unknown; value?: unknown }>)
...
async function fetchBangumiSubjectPersons(subjectId: string) {
...
async function fetchBangumiSubjectCharacters(subjectId: string) {
...
async function fetchBangumiSubjectRelations(subjectId: string) {
...
export async function getBangumiPersonDetail(personId: string | number) {
...
function preferChineseSummary(summary: string) {
...
function extractTags(subject: Record<string, unknown>) {
  ...
  return [
    ...
    ...tags.map((tag) => {
      const item = tag as { name?: unknown; count?: unknown };
...
function formatInfoboxValue(value: unknown): string {
  ...
  if (typeof value === "object" && value !== null) {
    if ("v" in value) return String((value as { v: unknown }).v || "");
...
export async function resolveMediaCover(mediaItemId: number) {
  const record = getCoverRecord(mediaItemId);
  if (!record) return null;
  if (record.cover_path && fs.existsSync(record.cover_path)) return record.cover_path;
  if (!record.cover_url || !record.external_id) return null;
  try {
...
async function cacheCover(url: string, fileBase: string) {
...
export async function getAnimeByTag(
  ...
  offset = 0, 
  limit = 50, 
  options?: { type?: number; sort?: string; airDate?: string[] }
  ...
  const json = await response.json() as { data?: Array<Record<string, unknown>>, total?: number };
...
export async function getPopularTags(): Promise<BangumiTag[]> {
  const FALLBACK_TAGS: BangumiTag[] = [
  ...
  try {
    ...
    const tagMap = new Map<string, BangumiTag>();
