...
const coverPathCache = new Map<number, string>();
...
app.whenReady().then(() => {
  ...
  const coverLocks = new Map<number, Promise<string | null>>();
  ...
  protocol.handle("cover", async (request) => {
    ...
    const servePath = async (p: string): Promise<Response | null> => {
...
function registerIpc() {
  ...
  ipcMain.handle("sources:addLocal", async (_, name: string) => {
    const result = await dialog.showOpenDialog({ properties: ["openDirectory"] });
    if (result.canceled || result.filePaths.length === 0) return null;
    return addLocalSourceByPath(result.filePaths[0], name || path.basename(result.filePaths[0]));
  });
  ipcMain.handle("sources:addLocalPath", (_, rootPath: string, name?: string) => addLocalSourceByPath(rootPath, name));
  ipcMain.handle("sources:addWebDav", (_, input: import("../shared/types.js").WebDavSourceInput) => addWebDavSourceAndTest(input));
  ipcMain.handle("sources:rename", (_, id: number, name: string) => renameSource(id, name));
  ipcMain.handle("sources:delete", (_, id: number) => deleteSource(id));
  ipcMain.handle("sources:scan", (_, sourceId: number) => {
    const source = listSources().find((item) => item.id === sourceId);
    if (source?.type === "webdav") return scanWebDavSource(sourceId, extensionList(getSettings().videoExtensions));
    return scanLocalSource(sourceId, extensionList(getSettings().videoExtensions));
  });

  ipcMain.handle("media:list", (_, args?: { search?: string; sort?: string; filter?: string }) =>
    listMediaItems(args?.search || "", args?.sort || "created_at", args?.filter || "all", (args as { sourceId?: number | null } | undefined)?.sourceId)
  );
  ipcMain.handle("media:get", (_, id: number) => ({ item: getMediaItem(id), files: listMediaFiles(id), candidates: listCandidates(id) }));
  ipcMain.handle("media:issues", (_, limit?: number) => listScrapeIssues(limit));
  ipcMain.handle("media:watchStats", () => getWatchStats());
  ipcMain.handle("media:setStatus", (_, id: number, status: import("../shared/types.js").WatchStatus) => setItemWatchStatus(id, status));
  ipcMain.handle("media:openFolder", async (_, folderPath: string) => {
  ...
  ipcMain.handle("files:play", (_, fileId: number) => playFile(fileId));
  ipcMain.handle("files:setWatched", (_, fileId: number, watched: boolean) => setFileWatched(fileId, watched));
  ipcMain.handle("files:setStatus", (_, fileId: number, status: string) => setFileStatus(fileId, status as import("../shared/types.js").WatchStatus));

  ipcMain.handle("scraper:searchBangumi", (_, mediaItemId: number, keyword?: string) => searchBangumi(mediaItemId, keyword));
  ipcMain.handle("scraper:applyBangumi", async (_, mediaItemId: number, externalId: string) => {
    const item = await applyBangumiCandidate(mediaItemId, externalId);
    coverPathCache.delete(mediaItemId);
    return item;
  });
  ipcMain.handle("scraper:getBangumiSubject", (_, subjectId: string) => getBangumiSubjectDetail(subjectId));
  ...
  ipcMain.handle("scraper:batchSearchBangumi", async (_, options?: { unmatchedOnly?: boolean; autoApplyThreshold?: number; delayMs?: number }) => {
    const result = await batchSearchBangumi(options);
    coverPathCache.clear();
    return result;
  });
  ipcMain.handle("scraper:batchRefreshBangumiById", async (_, options?: { delayMs?: number }) => {
  ...
  ipcMain.handle("season:getAnime", (_, year: number, season: string, options?: { refresh?: boolean }) =>
    getSeasonAnime(year, season as import("../shared/types.js").AnimeSeason, options)
  );
  ipcMain.handle("season:getDetail", (_, bangumiId: string | number) =>
    getBangumiSubjectDetail(String(bangumiId))
  );
  ipcMain.handle("season:getMikanResources", (_, keyword: string) =>
    searchMikan(keyword)
  );

  ipcMain.handle("online:listRules", () => listOnlineRules());
  ipcMain.handle("online:search", (_, input: import("../shared/types.js").OnlineRuleInput & { keyword: string }) => onlineSearch(input));
  ipcMain.handle("online:episodes", (_, input: import("../shared/types.js").OnlineRuleInput & { url: string }) => onlineEpisodes(input));
  ipcMain.handle("online:playUrl", async (_, url: string, title?: string, referer?: string, ruleInput?: import("../shared/types.js").OnlineRuleInput, bangumiId?: string | number) => {
  ...
  ipcMain.handle("logs:list", (_, limit?: number) => listLogs(limit));
  ...
  ipcMain.handle("subscriptions:items", (_, subscriptionId?: number | null) => subscriptionItems(subscriptionId));
  ipcMain.handle("subscriptions:add", (_, input: import("../shared/types.js").RssSubscriptionInput) => subscriptionAdd(input));
  ipcMain.handle("subscriptions:delete", (_, id: number) => subscriptionDelete(id));
  ipcMain.handle("subscriptions:searchMikan", (_, keyword: string) => searchMikan(keyword));
  ipcMain.handle("subscriptions:getMikanBangumi", (_, bangumiId: string | number) => getMikanBangumi(bangumiId));
  ipcMain.handle("subscriptions:refresh", (_, id: number) => refreshSubscription(id));
  ipcMain.handle("subscriptions:refreshAll", () => refreshAllSubscriptions());
  ipcMain.handle("subscriptions:sendItem", (_, itemId: number) => sendRssItemToQbittorrent(itemId));
  ipcMain.handle("subscriptions:sendUrl", (_, url: string, savePath?: string, title?: string, seriesTitle?: string) => sendUrlToQbittorrent(url, savePath, title, seriesTitle));
  ipcMain.handle("subscriptions:sendPending", (_, subscriptionId?: number | null) => sendPendingItemsToQbittorrent(subscriptionId));
  ipcMain.handle("subscriptions:testQbittorrent", () => testQbittorrent());
  ipcMain.handle("cloudOffline:list123", () => listPan123OfflineTasks());
  ipcMain.handle("cloudOffline:submit123", (_, input: { url: string; title?: string; dirId?: string }) => submitPan123OfflineDownload(input));
  ipcMain.handle("cloudOffline:refresh123", (_, taskId: string) => refreshPan123OfflineTask(taskId));
  ...
  ipcMain.handle("cloudOffline:submitPikpak", (_, input: { url: string; title?: string; savePath?: string }) => submitPikpakOfflineDownload(input));
...
async function checkLibrariesOnStartup(win: BrowserWindow) {
  ...
  for (const source of sources) {
    try {
      ...
      if (result.changed) changedSources += 1;
    } catch (error) {
      failed += 1;
