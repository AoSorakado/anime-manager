...
export function listCloudOfflineTasks(limit = 100): CloudOfflineTask[] {
  return getDb()
    .prepare("SELECT * FROM cloud_offline_tasks ORDER BY updated_at DESC LIMIT ?")
    .all(limit) as CloudOfflineTask[];
}
...
export function updateCloudOfflineTask(provider: CloudOfflineTask["provider"], taskId: string, patch: { progress?: number | null; status?: string | null; errorMessage?: string | null; rawJson?: string | null }) {
  getDb()
    .prepare(`
...
export function getCloudOfflineTask(provider: CloudOfflineTask["provider"], taskId: string): CloudOfflineTask | undefined {
  return getDb().prepare("SELECT * FROM cloud_offline_tasks WHERE provider = ? AND task_id = ?").get(provider, taskId) as CloudOfflineTask | undefined;
}

export function listRssSubscriptions(): RssSubscription[] {
  return getDb()
    .prepare(`
      SELECT rs.*,
...
export function getRssSubscription(id: number): RssSubscription | undefined {
  return getDb().prepare("SELECT * FROM rss_subscriptions WHERE id = ?").get(id) as RssSubscription | undefined;
}

export function addRssSubscription(input: RssSubscriptionInput): RssSubscription {
  const name = input.name.trim();
  const feedUrl = input.feedUrl?.trim() || buildMikanSearchFeed(input.keyword || name);
  if (!name) throw new Error("订阅名称不能为空");
  if (!feedUrl) throw new Error("RSS 地址不能为空");
  const result = getDb()
    .prepare(`
...
export function updateRssSubscription(id: number, patch: Partial<RssSubscriptionInput> & { enabled?: number }) {
  ...
  getDb()
    ...
    .run(
      nextName,
      nextFeedUrl,
      patch.keyword !== undefined ? patch.keyword.trim() || null : existing.keyword,
      patch.savePath !== undefined ? patch.savePath.trim() || null : existing.save_path,
...
export function deleteRssSubscription(id: number) {
  ...
  if (existing) log("info", "subscription", `删除 RSS 订阅：${existing.name}`, existing.feed_url);
  return { deleted: result.changes };
}

export function listRssItems(subscriptionId?: number | null, limit = 200): RssItem[] {
  ...
  return getDb()
    ...
    .all(...params) as RssItem[];
}

export function upsertRssItems(subscriptionId: number, items: Array<Omit<RssItem, "id" | "subscription_id" | "downloaded" | "download_status" | "created_at" | "updated_at">>) {
  ...
  getDb().transaction(() => {
    for (const item of items) {
      const result = stmt.run(
        ...
        item.guid,
        item.title,
        item.subtitle_group || null,
        item.size_text || null,
        item.updated_at_text || null,
        item.link || null,
        item.torrent_url || null,
        item.magnet_url || null,
        item.pub_date || null,
        item.raw_json || null,
        now(),
        now()
...
export function markRssItemDownloadResult(itemId: number, success: boolean, error?: string) {
  getDb()
...
export function getRssItem(itemId: number): RssItem | undefined {
  return getDb().prepare("SELECT * FROM rss_items WHERE id = ?").get(itemId) as RssItem | undefined;
}
...
export function log(level: LogLevel, module: LogModule, message: string, detail?: unknown, mediaItemId?: number) {
...
export function addLocalSource(name: string, rootPath: string): Source {
  const existing = getDb().prepare("SELECT * FROM sources WHERE type = 'local' AND root_path = ?").get(rootPath) as Source | undefined;
  if (existing) {
...
export function addLocalSourceByPath(rootPath: string, name?: string): Source {
...
export function addWebDavSource(input: WebDavSourceInput): Source {
  const name = input.name.trim();
  const webdavUrl = normalizeWebDavUrl(input.webdavUrl);
  const rootPath = normalizeWebDavPath(input.rootPath || "/");
  if (!name) throw new Error("WebDAV 显示名称不能为空");
  if (!webdavUrl) throw new Error("WebDAV 地址不能为空");
  const existing = getDb()
    .prepare("SELECT * FROM sources WHERE type = 'webdav' AND webdav_url = ? AND root_path = ?")
    .get(webdavUrl, rootPath) as Source | undefined;
...
export function renameSource(id: number, name: string) {
  const trimmed = name.trim();
  if (!trimmed) throw new Error("媒体库名称不能为空");
...
export function listMediaItems(search = "", sort = "created_at", filter = "all", sourceId?: number | null): MediaItem[] {
  const wheres: string[] = [];
  ...
  `;
  return getDb().prepare(sql).all(...params) as MediaItem[];
}
...
export function getCoverRecord(mediaItemId: number) {
  return getDb()
    .prepare(`
      ...
      WHERE mi.id = ?
    `)
    .get(mediaItemId) as { id: number; cover_path?: string | null; external_id?: string | null; provider?: string | null; cover_url?: string | null } | undefined;
}

export function updateCoverPath(mediaItemId: number, coverPath: string) {
...
export function repairMissingCovers() {
  const rows = getDb()
    ...
    `)
    .all() as Array<{ id: number; cover_path?: string | null; external_id: string; cover_url?: string | null }>;
...
export function exportLibrarySyncState(): LibrarySyncState {
  const items = getDb()
  ...
  const history = getDb()
    ...
    `)
    .all() as LibrarySyncState["history"];
  return {
    version: 1,
    exported_at: now(),
    items: items.map((item) => ({
      ...
      files: (filesStmt.all(item.id) as MediaFile[]).map((file) => ({
...
export function importLibrarySyncState(state: LibrarySyncState) {
  if (!state || state.version !== 1 || !Array.isArray(state.items)) throw new Error("同步文件格式不正确");
  ...
  getDb().transaction(() => {
    for (const incoming of state.items) {
      ...
      for (const incomingFile of incoming.files || []) {
        const file = findFile.get(item.id, incomingFile.file_name) as MediaFile | undefined;
        if (!file) continue;
    ...
    for (const incomingHistory of state.history || []) {
      ...
      if (!item) continue;
      const file = findFile.get(item.id, incomingHistory.file_name) as MediaFile | undefined;
      if (!file) continue;
      if (existingHistory.get(file.id, incomingHistory.played_at)) continue;
...
function findMatchingItem(
  incoming: { provider?: string | null; external_id?: string | null; clean_name?: string; folder_name: string },
  findItemByExternal: Database.Statement,
  findItemByName: Database.Statement
) {
  if (incoming.provider && incoming.external_id) {
    const byExternal = findItemByExternal.get(incoming.provider, incoming.external_id) as MediaItem | undefined;
    if (byExternal) return byExternal;
  }
  return findItemByName.get(incoming.clean_name || incoming.folder_name, incoming.folder_name) as MediaItem | undefined;
}

export function listCandidates(mediaItemId: number): MetadataCandidate[] {
  return getDb()
    .prepare("SELECT * FROM metadata_candidates WHERE media_item_id = ? ORDER BY match_score DESC NULLS LAST, score DESC NULLS LAST")
    .all(mediaItemId) as MetadataCandidate[];
}

export function listLogs(limit = 300): AppLog[] {
  return getDb().prepare("SELECT * FROM app_logs ORDER BY created_at DESC LIMIT ?").all(limit) as AppLog[];
}

export function listScrapeIssues(limit = 8): ScrapeIssue[] {
  return getDb()
    .prepare(`
      SELECT
        mi.id,
...
export function getWatchStats(): WatchStats {
  const row = getDb()
    .prepare(`
      ...
      FROM watch_history
    `)
    .get() as Omit<WatchStats, "recent" | "daily" | "top_titles" | "status_counts">;
  const recent = getDb()
    .prepare(`
      SELECT
        wh.id,
  ...
  const statusCounts = getDb()
    .prepare(`
      ...
      ORDER BY count DESC
    `)
    .all() as WatchStats["status_counts"];
  return {
...
export function recleanMediaItemNames() {
  const rows = getDb().prepare("SELECT id, folder_name, clean_name FROM media_items").all() as Array<{ id: number; folder_name: string; clean_name: string }>;
...
export function getSettings(): SettingsMap {
  ...
  return Object.fromEntries(rows.map((row) => [row.key, row.value])) as SettingsMap;
}
