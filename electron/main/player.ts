...
import type { MediaFile, WatchStatus } from "../shared/types.js";
import { updateBangumiSubjectStatus } from "./bangumiSync.js";

export function playFile(mediaFileId: number) {
  const file = getDb().prepare("SELECT * FROM media_files WHERE id = ?").get(mediaFileId) as MediaFile | undefined;
...
export function playUrl(url: string, title?: string, referer?: string, bangumiId?: string | number) {
...
async function monitorMpvPlayback({ child, pipeName, startedAt, historyId, file, mediaItemId, title, bangumiId, onlineTitle }: PlaybackMonitorOptions) {
  ...
  const finalize = async () => {
    ...
    if (file) {
      ...
      if (completed) refreshItemWatchStatus(file.media_item_id);
    }
    ...
    if (completed) {
      ...
      if (!bId && mediaItemId) {
        ...
        if (item?.provider === "bangumi") bId = item.external_id || undefined;
      }
      if (bId) {
        const settings = getSettings();
  ...
  try {
    ...
    const getProperty = (name: string) => new Promise<unknown>((resolve) => {
      if (!socket || socket.destroyed) return resolve(null);
      const id = requestId++;
    ...
    const poll = async () => {
      ...
      if (duration) state.mediaDuration = duration;
      ...
      if (position !== null) {
        if (state.lastPollAt !== null && state.lastPosition !== null && !state.lastPaused) {
          ...
          if (delta > 0 && wallDelta > 0 && wallDelta < 8) state.watchedSeconds += Math.min(wallDelta, delta + 1.2);
        }
...
function connectMpvIpc(pipeName: string) {
...
function windowTimeout(callback: () => void, ms: number) {
  return setTimeout(callback, ms);
}

export function setFileWatched(mediaFileId: number, watched: boolean) {
  const file = getDb().prepare("SELECT * FROM media_files WHERE id = ?").get(mediaFileId) as MediaFile | undefined;
  if (!file) throw new Error("Media file not found");
...
export function setItemWatchStatus(mediaItemId: number, status: WatchStatus) {
...
export function setFileStatus(mediaFileId: number, status: WatchStatus) {
  assertWatchStatus(status);
  const file = getDb().prepare("SELECT * FROM media_files WHERE id = ?").get(mediaFileId) as MediaFile | undefined;
  if (!file) throw new Error("Media file not found");
...
function refreshItemWatchStatus(mediaItemId: number) {
  const row = getDb()
    .prepare(`
      SELECT
        COUNT(*) AS total,
      ...
      WHERE media_item_id = ?
    `)
    .get(mediaItemId) as { total: number; unwatched: number; watched: number; watching: number; on_hold: number; dropped: number };
  
  let status: WatchStatus = "unwatched";
  if (row.total > 0 && row.dropped === row.total) status = "dropped";
  else if (row.total > 0 && row.on_hold === row.total) status = "on_hold";
  else if (row.total > 0 && row.watched === row.total) status = "watched";
  else if (row.watching > 0 || row.watched > 0 || row.on_hold > 0 || row.dropped > 0) status = "watching";

  getDb().prepare("UPDATE media_items SET watch_status = ?, updated_at = ? WHERE id = ?").run(status, now(), mediaItemId);
}

function assertWatchStatus(status: string): asserts status is WatchStatus {
